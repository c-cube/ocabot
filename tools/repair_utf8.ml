...
let is_utf8_char s =
  ...
  | `Uchar c -> Uutf.decode d = `End (* Uutf.cp_to_string c = s *)
  | `Malformed _ -> false
  | `Await -> assert false
  | `End -> false
...
let l =
  Uutf.String.fold_utf_8 (fun acc i c -> (i, c) :: acc) [] input |> List.rev
;;
...
let repair1 s : string option =
  let rec aux l =
    ...
    | c :: tail ->
      ...
      aux tail >|= fun tl -> c :: tl
  in
  CCString.to_list s |> aux
